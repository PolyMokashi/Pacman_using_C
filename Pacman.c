...
void user_input()
{
   if (_kbhit())
   {
      ...
      if (c1 == -32)
      {
         ...
         myPacMan.vy = 0;
         // printf("c1=%d c2=%d\n", c1, c2);
         // for ghost c2 is of random numbers; c1 is always -32;7
         switch (c2)
         {
         case 72:
            myPacMan.vy = -1;
...
void show_playfield()
{
   ...
   printf("Score: %d\n", myPacMan.food_collected);
         ...
         else if (myPacMan.vy == +1)
         {
            printf("\nMove v\n");
         }
         else if (myPacMan.vx == -1)
         {
            printf("\nMove <\n");
         }
         else if (myPacMan.vx == +1)
...
int main()
{
   ...
   while (1)
   {
      ...
      if (playfield[ny][nx] == '&')
      {
         ...
         system("cls");
         printf("Game Over\n");
         printf("Your Score is:%d", myPacMan.food_collected);
         break;
      }

      // 6. put PacMan back again to playfield
      playfield[myPacMan.position.y][myPacMan.position.x] = 'C';
